window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			title: "顾问 / 第二意见",
			description: "默认走官方 DeepSeek（稳定、身份可验证）。想换供应商时在下方指定。"
		};
		const en = {
			title: "Advisor / Second Opinion",
			description: "Defaults to official DeepSeek (stable, verifiable). Pick a provider below to override."
		};
		...
		function AdvisorSection(props) {
			...
			return jsx.jsxs("div", { style: S.section, children: [
				...
				jsx.jsxs("p", { style: S.hint, children: [
					"默认路由：", cfg && cfg.defaultRoute ? cfg.defaultRoute : "deepseek-official/deepseek-v4-flash",
