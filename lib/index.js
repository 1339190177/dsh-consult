...
const BODY_LIMIT_BYTES = 16 << 20
const LOOPBACKS = new Set(['127.0.0.1', '::1', '::ffff:127.0.0.1'])
// 唯一权威模型调用超时（v2.4：30 分钟；DSH_CONSULT_TIMEOUT_MS 可调，下限 10s 防手滑）
const MODEL_CALL_TIMEOUT_MS = Math.max(10_000, Number(process.env.DSH_CONSULT_TIMEOUT_MS) || 1_800_000)
// v2.2：consult 工具不再设 maxTokens 上限——答案长度由 system 提示约束（≤300字），
// 混合推理模型的思考与正文共享预算，任何上限都可能把思考饿死成空意见。
...
async function consultScout(question, context, { signal } = {}) {
  ...
  const resp = await fetch(SCOUT_ENDPOINT, {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 8000,  // 思考与正文共享预算；正文由 system 约束 ≤400字，上限只为防失控
      system: SCOUT_SYSTEM,
...
//#region 官方视觉直连（v2.4 多模态：web 组合无附件服务时的图片通道）

/** 官方 Anthropic 兼容端点直连：base64 图片 + 文本 → 视觉模型意见。
 *  与 scout 同通道同范式（readDeepSeekOfficialKey 复用）；超时由调用方的
 *  AbortController 管。images 为 mapOpenAiMessages 收集的 {input:{mediaType,data}}。 */
async function consultVision(system, text, images, model, signal) {
  const key = readDeepSeekOfficialKey()
  if (!key) throw Object.assign(
    new Error('图片咨询需要 DEEPSEEK_API_KEY（环境变量或 ~/.dsh/.credentials.yaml）'),
    { statusCode: 503 })
  const content = images.map((img) => ({
    type: 'image',
    source: { type: 'base64', media_type: img.input.mediaType, data: img.input.data },
  }))
  content.push({ type: 'text', text: text === '' ? '请分析随附图片。' : text })
  ...
  if (!resp.ok) throw Object.assign(
    new Error(`视觉上游 ${resp.status}: ${(await resp.text()).slice(0, 200)}`),
    { statusCode: 502 })
  const data = await resp.json()
  const opinion = (data.content ?? []).filter((b) => b.type === 'text').map((b) => b.text).join('\n').trim()
  return { opinion, usage: data.usage }
}
...
function registerConsultTool(ctx, deps) {
  const tool = defineTool({
    ...
    description:
      ...
      '不适用：能从本项目代码直接推断的问题。' +
      '默认路由 = 官方 DeepSeek（deepseek-official/deepseek-v4-flash，稳定且身份可验证）；' +
      '可在 settings 面板或 advisor.route 配置，也可显式传 model="provider/model"。' +
...
/** 解析 data:image/...;base64,... 为 {mediaType, data}。仅接受 data URL——
 *  http(s) URL 是 SSRF 面（回环端点去抓任意 URL 不可接受），明确拒绝。 */
...
/**
 * OpenAI messages → dsh Message 列表 + system。
 * v2.4 多模态：user content 数组里的 `image_url` 部件（data URL）被收集为
 * `images: [{msgIndex, input}]`，由调用方经附件库落盘后追加 ImageBlock。
 * 不认识的部件类型显式 400（不再静默丢弃）。
 */
  ...
  for (const message of input) {
    ...
    if (typeof message.content === 'string') text = message.content
      ...
      for (const part of message.content) {
        ...
        } else if (part !== null && typeof part === 'object' && part.type === 'image_url') {
          const url = part.image_url?.url ?? part.image_url
          const parsed = parseDataImageUrl(url)
          if (parsed === undefined) {
            if (typeof url === 'string' && /^https?:/i.test(url)) {
              throw Object.assign(new Error('image_url 仅支持 data: URL（base64 内联）；http(s) URL 请先下载为 data URL 再发'), { statusCode: 400 })
            }
            throw Object.assign(new Error('image_url 需为 {url: "data:image/png;base64,..."}（支持 png/jpeg/webp/gif）'), { statusCode: 400 })
          }
          images.push({ msgIndex: messages.length, input: { mediaType: parsed.mediaType, data: parsed.data, name: 'advisory-image' } })
        } else if (part !== null && typeof part === 'object' && part.type !== 'text') {
          throw Object.assign(new Error(`不支持的 content 部件类型 "${part.type}"（支持 text / image_url）`), { statusCode: 400 })
        }
      }
      text = parts.join('')
    } else if (message.content !== undefined && message.content !== null) {
      throw Object.assign(new Error('`content` 仅支持 string 或部件数组（text/image_url）'), { statusCode: 400 })
    }
...
function registerHttpEndpoint(ctx, deps) {
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      let abortTimer = null // 提前声明：外层 catch 可能早于其 const 初始化触发（TDZ 崩溃的教训）
      try {
        ...
        // v2.4 多模态：图片直连官方视觉模型（Anthropic 兼容端点原生吃 base64）。
        // 为什么不走 llm.stream：web 组合不挂载附件服务（attachments 只在 agent
        // 会话内可用），内联 ImageBlock 无法落库；直连官方与 scout 同通道同范式。
        if (mapped.images.length > 0) {
          if (route.provider !== 'deepseek-official') {
            sendJson(400, openaiError(`图片目前仅支持官方路由（deepseek-official），当前 ${route.provider} 不支持`, 'invalid_request_error', 'image_provider_unsupported'))
            return
          }
          const info = await deps.llm.resolveModelInfo(route.provider, route.model).catch(() => undefined)
          if (info?.inputModalities?.includes('image') !== true) {
            if (route.explicit) {
              sendJson(400, openaiError(`模型 ${route.model} 不支持图片输入；请选择声明 image 输入的模型（如 deepseek-v4-flash-vision-exp）`, 'invalid_request_error', 'model_not_image_capable'))
              return
            }
            const models = await deps.llm.listModels(route.provider).catch(() => [])
            let switched = undefined
            for (const m of models) {
              const mi = await deps.llm.resolveModelInfo(route.provider, m.id).catch(() => undefined)
              if (mi?.inputModalities?.includes('image')) { switched = m.id; break }
            }
            if (switched === undefined) {
              sendJson(400, openaiError('官方目录里没有声明 image 输入的模型，无法处理图片', 'invalid_request_error', 'no_image_model'))
              return
            }
            console.log(NAME, `：请求含图片，${route.model} 不支持视觉，自动升级 → ${switched}`)
            headers['x-advisor-vision'] = `${route.provider}/${route.model} -> ${route.provider}/${switched}`
            route = { ...route, model: switched }
          }
          const transcript = mapped.messages
            .map((m) => m.content.map((b) => b.text ?? '').join(''))
            .filter((t) => t !== '')
            .join('\n')
          const startedVision = Date.now()
          const visionAbort = new AbortController()
          abortTimer = setTimeout(() => visionAbort.abort(), MODEL_CALL_TIMEOUT_MS)
          try {
            const vision = await consultVision(system ?? '你是技术顾问，结合随附图片回答。', transcript, mapped.images, route.model, visionAbort.signal)
            headers['x-model-upstream'] = `deepseek-official/${route.model}+vision`
            audit({
              ts: new Date().toISOString(), source: 'http',
              question: truncate(transcript, 200),
              route: { provider: route.provider, model: route.model },
              images: mapped.images.length,
              durationMs: Date.now() - started, ok: true, structured: false, adopted: null,
            })
            sendJson(200, {
              id: `chatcmpl-${randomUUID()}`, object: 'chat.completion', created: Math.floor(Date.now() / 1000),
              model: `${route.provider}/${route.model}`,
              choices: [{ index: 0, message: { role: 'assistant', content: vision.opinion }, finish_reason: 'stop' }],
              usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0, ...(vision.usage ?? {}) },
              advisor: { images: mapped.images.length },
            })
          } catch (error) {
            if (visionAbort.signal.aborted) {
              sendJson(504, openaiError(`模型调用超时（${Math.round(MODEL_CALL_TIMEOUT_MS / 1000)}s，已中止）`, 'api_error', 'upstream_timeout'))
            } else {
              sendJson(502, openaiError(`视觉顾问调用失败：${String(error?.message ?? error).slice(0, 160)}`, 'api_error', 'vision_failed'))
            }
          } finally {
            if (abortTimer !== null) clearTimeout(abortTimer)
          }
          console.log(NAME, `：图片咨询完成 ${mapped.images.length} 张，${Date.now() - startedVision}ms`)
          return
        }

        // 服务端超时：上游挂起时中止模型调用（实测官方路由偶发无限挂起——
        // 没有它，客户端断开后 handler 永远 pending 且可能卡住 provider 资源）。
        // v2.4 放宽到 30 分钟（1800s）给深度调查类请求留余量。
        const abort = new AbortController()
        abortTimer = setTimeout(() => abort.abort(), MODEL_CALL_TIMEOUT_MS)
        const options = {
          provider: route.provider,
          model: route.model,
...
/** 供面板读取的供应商目录：id + 模型列表（含 display name 与该模型支持的
 *  推理档位，档位来自 resolveModelInfo——与 dsh 自家模型选择器同一数据源）。 */
async function providerCatalog(llm) {
  const out = []
  for (const p of llm.listProviders()) {
    let models = []
    try {
      models = await Promise.all((await llm.listModels(p.id)).map(async (m) => {
        let efforts
        try {
          efforts = (await llm.resolveModelInfo(p.id, m.id))?.reasoning?.efforts?.map((e) => e.id)
        } catch { efforts = undefined }
      ...
      try {
        ...
        if (req.method === 'GET') {
          ...
          return sendJson(res, 200, {
            ...
            defaultRoute: parseRoute(DEFAULT_ROUTE),
            endpoint: HTTP_PATH,
          })
        }
        if (req.method === 'POST') {
