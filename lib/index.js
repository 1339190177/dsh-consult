...
// 唯一权威模型调用超时（v2.3：240s；DSH_CONSULT_TIMEOUT_MS 可调，下限 10s 防手滑）
const MODEL_CALL_TIMEOUT_MS = Math.max(10_000, Number(process.env.DSH_CONSULT_TIMEOUT_MS) || 240_000)
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        // 正常复杂问答 ~65s；v2.3 放宽到 240s 给深度思考留余量。
        const abort = new AbortController()
        const abortTimer = setTimeout(() => abort.abort(), MODEL_CALL_TIMEOUT_MS)
        const options = {
          provider: route.provider,
          model: route.model,
