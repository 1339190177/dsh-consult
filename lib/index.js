...
/** 供应商兜底链（稳定性）：主路由硬失败（超时/网络/上游5xx）时按序换家重试。
 *  deepseek(XQAPI中转) → deepseek-official(官方，异源且身份可靠) → zai(同源末位)。
 *  响应里显式标记兜底路径，绝不静默冒充异源第二意见。只兜底硬故障，不兜底模型拒答。 */
const FALLBACK_ROUTES = {
  deepseek: ['deepseek-official/deepseek-v4-flash', 'zai-coding-cn/glm-4.5-air'],
}

function isHardFailure(error) {
  const msg = String(error?.message ?? error)
  return /timed? ?out|TIMEOUT|ECONNRESET|ECONNREFUSED|ENOTFOUND|EAI_AGAIN|fetch failed|socket hang up|502|503|504/i.test(msg)
}
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (!wantsStream) {
          // 供应商兜底链：主路由硬故障（超时/网络）→ 按序换家，响应带显式标记
          let result
          let usedFallback = false
          let effModelLabel = modelLabel
          const fallbacks = (FALLBACK_ROUTES[route.provider] ?? [])
            .map((r) => parseRoute(r)).filter((r) => r !== undefined)
          try {
            result = assertStreamOk(await pullStream(deps.llm.stream(options)))
          } catch (error) {
            if (!isHardFailure(error) || fallbacks.length === 0) throw error
            let lastError = error
            result = undefined
            for (const fb of fallbacks) {
              try {
                usedFallback = true
                headers['x-advisor-fallback'] = `${route.provider}/${route.model} -> ${fb.provider}/${fb.model}`
                console.log(NAME, `：主路由 ${route.provider}/${route.model} 硬故障（${String(lastError?.message ?? lastError).slice(0, 80)}），兜底 → ${fb.provider}/${fb.model}`)
                effModelLabel = `${fb.provider}/${fb.model}`
                result = assertStreamOk(await pullStream(deps.llm.stream({
                  ...options, provider: fb.provider, model: fb.model,
                })))
                break
              } catch (fbError) {
                lastError = fbError
                if (!isHardFailure(fbError)) throw fbError
                console.log(NAME, `：兜底 ${fb.provider}/${fb.model} 也硬故障（${String(fbError?.message ?? fbError).slice(0, 80)}），试下一家`)
              }
            }
          ...
          const payload = {
            ...
            ...(usedFallback ? { advisor: { fallback: true, from: modelLabel, to: effModelLabel } } : {}),
          }
          finish()
